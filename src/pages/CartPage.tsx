...
import { saveOrder, getSession, generateToken, generateId, now, today, addPointsApi, getLoyalty, redeemPointsApi, pointsToDiscount, formatPrice } from "@/data/storage";
      ...
      <div className="px-4 pt-4 max-w-lg mx-auto">
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">You have {loyalty.points} pts → saves {formatPrice(pointsToDiscount(Math.min(loyalty.points, 500)))}</p>
                </div>
              </div>
              <button
        ...
        <div className="bg-card rounded-2xl p-5 card-shadow border border-border">
          ...
          <div className="space-y-2 text-sm">
            <div className="flex justify-between"><span className="text-muted-foreground">Subtotal</span><span className="font-bold">{formatPrice(totalPrice)}</span></div>
            <div className="flex justify-between"><span className="text-muted-foreground">Service Tax (5%)</span><span className="font-bold">{formatPrice(tax)}</span></div>
            {usePoints && pointsDiscount > 0 && (
              <div className="flex justify-between"><span className="text-primary flex items-center gap-1"><Tag className="w-3 h-3" /> Points Discount</span><span className="font-bold text-primary">-{formatPrice(pointsDiscount)}</span></div>
            )}
            <div className="border-t border-border my-3" />
            <div className="flex justify-between text-xl"><span className="font-extrabold">Total</span><span className="font-extrabold text-primary">{formatPrice(total)}</span></div>
          </div>
        </div>
